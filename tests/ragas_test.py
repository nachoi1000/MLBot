...
# Carga tus variables de entorno (necesario para la API de OpenAI)
load_dotenv()

# 1. Prepara tus datos de prueba (usa la variable `test_data` del paso anterior)
test_data = [
    # ... (pega aquí tu lista de diccionarios generada) ...
]

# 2. Convierte tu lista de diccionarios a un formato que RAGAS entiende: un Dataset de Hugging Face
...
# 3. Define las métricas que quieres calcular
metrics = [
    faithfulness,
    answer_relevancy,
    context_precision,
...
# RAGAS usará un LLM (por defecto gpt-3.5-turbo-16k) para calificar las respuestas
print("Ejecutando la evaluación con RAGAS...")
result = evaluate(
...
# Para una vista más amigable, puedes convertirlo a un DataFrame de pandas
# df = result.to_pandas()
# print(df)